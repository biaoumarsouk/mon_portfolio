  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-300 selection:bg-blue-500/30 overflow-x-hidden font-sans">
      ...
      <AnimatePresence>
        ...
        {/* MODAL CONTACT "FULL EXPERIENCE" */}
        {isContactOpen && (
          <div className="fixed inset-0 z-[200] flex justify-center items-center p-4 bg-slate-950/98 backdrop-blur-3xl overflow-y-auto">
             <motion.div 
                initial={{ scale: 0.9, opacity: 0, y: 40 }} 
                animate={{ scale: 1, opacity: 1, y: 0 }} 
                exit={{ scale: 0.9, opacity: 0, y: 40 }} 
                className="bg-slate-900 border border-white/10 rounded-[40px] md:rounded-[60px] w-full max-w-6xl relative flex flex-col md:flex-row overflow-hidden shadow-2xl my-8"
             >
                ...
                <div className="md:w-2/5 bg-blue-600 p-10 md:p-16 text-white flex flex-col justify-between relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <h2 className="text-4xl md:text-6xl font-black italic uppercase tracking-tighter mb-8 leading-none">
                      Parlons <br /> de votre <br /> <span className="underline decoration-white/20 underline-offset-8">Projet.</span>
                    </h2>
                    <p className="text-blue-100 text-lg md:text-xl font-medium italic mb-12 opacity-80 leading-relaxed">
                      Disponible pour des missions en Réseau, Sécurité ou Développement Fullstack.
                    </p>
                  </div>

                  <div className="space-y-8 relative z-10 font-bold uppercase text-[10px] tracking-[0.2em]">
                    <div className="flex items-center gap-6 group cursor-pointer">
                      <div className="p-4 bg-white/10 rounded-2xl group-hover:bg-white group-hover:text-blue-600 transition-all duration-500">
                        <Mail size={24} />
                      </div>
                      <div>
                        <p className="opacity-50 mb-1">Email professionnel</p>
                        <p className="text-base font-black lowercase tracking-normal italic">{portfolioData.contact.email}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-6 group cursor-pointer">
                      ...
                      <div>
                        ...
                        <p className="text-base font-black italic">{portfolioData.contact.localisation}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="md:w-3/5 p-10 md:p-20 relative bg-slate-900 flex flex-col justify-center">
                  <button onClick={() => setIsContactOpen(false)} className="absolute top-8 right-8 text-slate-500 hover:text-white transition cursor-pointer p-3 bg-white/5 rounded-full">
                    <X size={32} />
                  </button>
                  
                  <form className="space-y-8 italic font-bold">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-2">
                        <label className="text-[10px] uppercase font-black text-blue-500 tracking-widest ml-2 italic">Nom & Prénom</label>
                        <input type="text" className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-5 outline-none focus:border-blue-600 text-white text-lg transition-all shadow-inner" placeholder="Marsouk ..." />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] uppercase font-black text-blue-500 tracking-widest ml-2 italic">Votre Email</label>
                        <input type="email" className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-5 outline-none focus:border-blue-600 text-white text-lg transition-all shadow-inner" placeholder="email@domaine.com" />
                      </div>
                    </div>

                    <div className="space-y-2">
                      <label className="text-[10px] uppercase font-black text-blue-500 tracking-widest ml-2 italic">Sujet de discussion</label>
                      <input type="text" className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-5 outline-none focus:border-blue-600 text-white text-lg transition-all shadow-inner" placeholder="Ex: Audit Réseau / Application React..." />
                    </div>

                    <div className="space-y-2">
                      <label className="text-[10px] uppercase font-black text-blue-500 tracking-widest ml-2 italic">Votre Message</label>
                      <textarea rows="5" className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-5 outline-none focus:border-blue-600 text-white text-lg transition-all shadow-inner resize-none" placeholder="Dites-moi tout..."></textarea>
                    </div>

                    <motion.button 
                      whileTap={{ scale: 0.95 }}
                      className="w-full bg-blue-600 py-6 rounded-3xl font-black text-white uppercase tracking-[0.3em] text-xs hover:bg-blue-500 transition-all shadow-2xl shadow-blue-600/30 flex items-center justify-center gap-4 group"
                    >
